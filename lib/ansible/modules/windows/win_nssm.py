...
options:
  ...
  start_mode:
    description:
      - If C(auto) is selected, the service will start at bootup. C(delayed) causes a delayed but automatic start after boot. C(manual) means that the service will start only when another service needs it.
